...
int main(int argc, char* argv[])
{
	...
	double sinr_dtpc[round_bound][n][no_specs];
	double t_sinr_dtpc_l[n][no_specs];
	double t_sinr_dtpc_n[n][no_specs];
	double sinr[round_bound][n][no_specs];
	double t_sinr_l[n][no_specs];
	double t_sinr_n[n][no_specs];
	double sum_sinr_dtpc[round_bound];
	double sum_sinr[round_bound];
	int sinr_received_counter = 0;

	double p_dtpc[round_bound][n][no_specs];
	double t_p_dtpc_l[n][no_specs];
	double t_p_dtpc_n[n][no_specs];
	double p[round_bound][n][no_specs];
	double t_p_n[n][no_specs];
	double t_p_l[n][no_specs];
	double sum_power[round_bound];
	double sum_power_dtpc[round_bound];

	double I[round_bound][n][no_specs];
	double t_I_l[n][no_specs];
	double t_I_n[n][no_specs];
	double I_dtpc[round_bound][n][no_specs];
	double t_I_dtpc_n[n][no_specs];
	double t_I_dtpc_l[n][no_specs];
	...
	while (round < round_bound && round_loop_flag != 1)
	{
		...
		sum_sinr[round] = 0;
		sum_power[round] = 0;
		sum_sinr_dtpc[round] = 0;
		sum_power_dtpc[round] = 0;
		for (int i = 1; i<n; i++)
		{
		...
		iteration = 0;
		// DTPC power control procedure
		for (int i = 1; i < n; i++)
		{
			for (int k = 0; k < no_specs; k++)
				t_p_dtpc_n[i][k] = 0;
			t_p_dtpc_n[i][channel[round][i]] = max_power;
		}
		...
		while (iteration < iteration_bound)
		{
			if (iteration != 0)
				for (int i = 1; i < n; i++)
				{
					for (int k = 0; k < no_specs; k++)
						t_p_dtpc_n[i][k] = 0;
					t_p_dtpc_n[i][channel[round][i]] = min(max_power, max((target_sinr*(t_p_dtpc_l[i][channel[round][i]] / t_sinr_dtpc_l[i][channel[round][i]])), eta*(t_sinr_dtpc_l[i][channel[round][i]] / t_p_dtpc_l[i][channel[round][i]])));
				}
			...
			for (int i = 1; i < n; i++)
				for (int k = 0; k < no_specs; k++)
				{
					...
					for (int j = 1; j < n; j++)
						if (j != i && j != next_node[i])
							t_I_dtpc_n[i][k] = t_I_dtpc_n[i][k] + t_p_dtpc_n[j][k] * h(x[j], x[next_node[i]], y[j], y[next_node[i]]);
					t_I_dtpc_n[i][k] = t_I_dtpc_n[i][k] + noise;
					t_sinr_dtpc_n[i][k] = (t_p_dtpc_n[i][k] * h(x[i], x[next_node[i]], y[i], y[next_node[i]])) / t_I_dtpc_n[i][k];
				}

			for (int i = 1; i < n; i++)
				for (int k = 0; k < no_specs; k++)
				{
					t_p_dtpc_l[i][k]=t_p_dtpc_n[i][k];
					t_I_dtpc_l[i][k]=t_I_dtpc_n[i][k];
					t_sinr_dtpc_l[i][k]=t_sinr_dtpc_n[i][k];
				}
		...
		for (int i = 1; i < n; i++)
			for (int k = 0; k < no_specs; k++)
			{
				p_dtpc[round][i][k] = t_p_dtpc_l[i][k];
				I_dtpc[round][i][k] = t_I_dtpc_l[i][k];
				sinr_dtpc[round][i][k] = t_sinr_dtpc_l[i][k];
			}
		for (int i = 1; i<n; i++)
			for (int k = 0; k<no_specs; k++)
			{
				sum_sinr_dtpc[round] = sum_sinr_dtpc[round] + sinr_dtpc[round][i][k];
				sum_power_dtpc[round] = sum_power_dtpc[round] + p_dtpc[round][i][k];
			}
		...
		/*for (int i = 1; i < n; i++)
		{
			if (sinr_dtpc[round][i][channel[round][i]] >= target_sinr)
		...
		for (int i = 1; i < n;i++)
			cout << sinr_dtpc[round][i][channel[round][i]] << " ";
		cout << "\n";

		cout << sum_power_dtpc[round] << "\n";
		cout << sum_sinr_dtpc[round] << "\n";
	...
	ofstream file_SoP;
	file_SoP.open("R02_DTPC_SoP.txt", std::ios::app);
	file_SoP << sum_power_dtpc[round-1] << "\n";
	file_SoP.close();

	// writing Sum of SINR to file
	ofstream file_SoS;
	file_SoS.open("R03_DTPC_SoSNR.txt", std::ios::app);
	file_SoS << sum_sinr_dtpc[round-1] << "\n";
	file_SoS.close();

	// writingNumber of Satisfied Users to file
	ofstream file_et;
	file_et.open("R04_DTPC_NoSU.txt", std::ios::app);
